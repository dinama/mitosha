...
avlnode_t *avltree_next(const avlnode_t *node)
{
  ...
  while ((r = get_parent_avl(node)) &&
         (mvoid_get(&r->right) == node) ) {
    node = r;
  }
 ...
 * lookups.
 */
static inline avlnode_t *do_lookup_avl(const avlnode_t *key,
                                       avltree_compare_f cmp,
                                       const avltree_t *tree,
                                       avlnode_t **pparent,
                                       avlnode_t **unbalanced,
                                       int *is_left)
{
...
avlnode_t *avltree_lookup(const avlnode_t *key,
                      avltree_compare_f cmp,
                      const avltree_t *tree)
{
...
avlnode_t *avltree_lower(const avlnode_t *key,
                         avltree_compare_f cmp,
                         const avltree_t *tree)
{
  int rc = 0;
...
avlnode_t *avltree_upper(const avlnode_t *key,
                         avltree_compare_f cmp,
                         const avltree_t *tree)
{
  avlnode_t * r = avltree_lower(key, cmp, tree);
  while (r && 0 == cmp(r, key)) r = avltree_next(r);
  return r;
}


inline static void set_child_avl(avlnode_t *child,
                                 avlnode_t *node,
                                 int left)
{
  if (left) mvoid_set(&node->left , child);
  else mvoid_set(&node->right, child);
}

/* Insertion never needs more than 2 rotations */
avlnode_t *avltree_insert(avlnode_t *node,
                      avltree_compare_f cmp,
                      avltree_t *tree)
{
